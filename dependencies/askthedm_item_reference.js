...
function getRandomItem(category, rarity) {
    ...
    // If items are weighted (Arcana/Common or Arcana/Uncommon), roll d100 and find matching item
    if (category === 'Arcana' && (rarity === 'Common' || rarity === 'Uncommon')) {
        let roll = Math.floor(Math.random() * 100) + 1;
        let found = items.find(item => roll >= item.min && roll <= item.max);
