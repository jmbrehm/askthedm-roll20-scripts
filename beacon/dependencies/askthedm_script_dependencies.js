...
//           askTheDMGetAttribute, askTheDMSetAttribute, askTheDMParseRollResult

// Display load message in the Mod Output Console
on('ready', function() {
    log('========================================');
    log('askthedm_script_dependencies.js loaded (version 3.0)');
    log('Provides: ITEM_TABLES, getRandomItem, CR_ATTRIBUTE_REFERENCE, DamageToSave, DamageToVEffect, DarkvisionBySpecies');
    log('Beacon helpers: askTheDMGetAttribute, askTheDMSetAttribute, askTheDMParseRollResult');
    log('========================================');
});

// --- Beacon Compatibility Helpers (Beacon-only) ---
// These wrappers require Beacon async APIs and intentionally do not fall back to legacy attributes.

function askTheDMNormalizeSheetItemName(name, opts) {
    if (!name || typeof name !== 'string') return name;
    if (name.indexOf('user.') === 0) return name;
    if (opts && opts.userDefined === true) return 'user.' + name;
    return name;
}

async function askTheDMGetAttribute(characterId, attributeName, property, opts) {
    if (!characterId || !attributeName) return null;

    let key = askTheDMNormalizeSheetItemName(attributeName, opts);
    let prop = property === 'max' ? 'max' : undefined;
    ...
    let result = await getSheetItem(characterId, key, prop);
    return result === undefined ? null : result;
}

async function askTheDMSetAttribute(characterId, attributeName, value, property, opts) {
    ...
    if (prop) {
        return setSheetItem(characterId, key, value, prop);
    }
    return setSheetItem(characterId, key, value);
}

function askTheDMParseRollResult(msgContent) {
    if (!msgContent || typeof msgContent !== 'string') return null;
    let match = msgContent.match(/data-result="(.+?)"/i);
    return match ? match[1] : null;
}

globalThis.askTheDMGetAttribute = askTheDMGetAttribute;
globalThis.askTheDMSetAttribute = askTheDMSetAttribute;
globalThis.askTheDMParseRollResult = askTheDMParseRollResult;
